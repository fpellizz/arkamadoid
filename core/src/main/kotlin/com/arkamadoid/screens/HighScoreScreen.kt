...
class HighScoreScreen(game: ArkamadoidGame) : BaseScreen(game) {
    ...
    override fun render(delta: Float) {
        ...
        layout.setText(wipFont, "DATA_STORE EMPTY")
        wipFont.draw(batch, "DATA_STORE EMPTY", (VIRTUAL_W - layout.width) / 2f, VIRTUAL_H / 2f + 40f)

        val sub = game.fonts[Theme.FontSize.BODY_MD]
        sub.color = Theme.Palette.ON_SURFACE_VARIANT
        layout.setText(sub, "leaderboard coming soon")
        sub.draw(batch, "leaderboard coming soon", (VIRTUAL_W - layout.width) / 2f, VIRTUAL_H / 2f - 20f)
